-- CRITICAL SECURITY FIX: Secure employee data and hash passwords
...
-- Create new restrictive policies that work with our custom auth
-- Note: Since we use custom auth via localStorage, we need different approach
-- These policies will effectively deny all direct access and force use of RPC functions

-- Policy for employees to view only their own basic info (excluding password)
CREATE POLICY "Employees view own info via RPC only" ON public.employees
FOR SELECT 
USING (false); -- Deny all direct access, force RPC usage

-- Policy for HR operations via RPC only  
CREATE POLICY "HR operations via RPC only" ON public.employees
FOR ALL
USING (false) -- Deny all direct access, force RPC usage
WITH CHECK (false);
...
-- Create restrictive policies for referrals - force RPC usage only
CREATE POLICY "Referrals access via RPC only" ON public.referrals
FOR ALL
USING (false)
WITH CHECK (false);
